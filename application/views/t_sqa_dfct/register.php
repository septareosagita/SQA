...
<script type="text/javascript">
    ...
    // fungsi untuk mengambil vinno yang dipilih jika body no memiliki banyak vinno
    // created by: irfan.satriadarma@gmail.com on 201205080939pm
    function on_select_vinno_id() {
        var vinno_id = $('#vinno_id').val();
        if (vinno_id != '0') {
			$('#vinno').val(vinno_id);
			$('#vinno').show();
			$('#panel_vinno').html('');
    ...
    function get_vehicle(by) {
        ...
        function(html) {
            ...
            if (html==0){
				...
				if (html.substr(0,11) == 'FM_VINNO;;;') {

					// FOUND more than one VINNO by this 									
					var ex_vinno = html.split(';;;');
					$('#vinno').hide();
					$('#panel_vinno').html(ex_vinno[1]);
					
				} else {
					// kembalikan vinno seperti semula
					$('#vinno').show();
					$('#panel_vinno').html('');
	                ...
	                if (auditor_login == auditor_nm_1 || auditor_login == auditor_nm_2 || auditor_groupauth =='05' || auditor_groupauth == '11') {
	                    // boleh manage defect
	                } else {
    ...
    //function get_vinf_under_sqa(bodyno) { * edited by: irfan.satriadarma@gmail.com 20120504 -> jgn berdasarkan bodyno tapi vinno
    function get_vinf_under_sqa(vinno) {
    ...
    function get_dfct(problem_id) {        
		...
		// edited by irfan@arkamaya.com on 20120911
		// pengambilan defect belum disertakan dengan perbandingan VINNO nya
		var vinno	= $('#vinno').val();

        // $.post('<?=site_url('t_sqa_dfct/get_dfct')?>', {body_no: body_no,problem_id: problem_id}, function(html){
		$.post('<?=site_url('t_sqa_dfct/get_dfct')?>', {body_no: body_no,vinno: vinno, problem_id: problem_id}, function(html){
    ...
    function get_dfct_by_problem_id() {
        ...
        $.post('<?=site_url('t_sqa_dfct/get_dfct_by_problem_id')?>', {problem_id:problem_id}, function(html){
            if (html == 0) {
                // bersihkan defect
                //clear_defect();
                get_vehicle('vinno');//get_vehicle('body_no');
            } else {                
                $('#dfct_panel').show();                
                t_dfct = 1;
                ...
                */
                ...
                var auditor_login = '<?=$auditor_nm_1?>';
                var auditor_groupauth = '<?=get_user_info($this, 'GRPAUTH_ID')?>';
                if (auditor_login == auditor_nm_1 || auditor_login == auditor_nm_2 || auditor_groupauth == '05' || auditor_groupauth == '11' || auditor_groupauth == '09' || auditor_groupauth == '08') {
                    // boleh manage defect
                    if (
                            // notes: 20111124 --> masih bisa di edit kalau belum di approve.
    ...
    function dfct_button_1() {
        // cek jika sudah finish dari tombol finish audit dulu, klo sdh finish dia tdk boleh di add        
        var cek = $('#btnFinishAudit').attr('disabled');
        if (cek == false) {
            $('#btnAdd').removeAttr('disabled');    
        }
    ...
    function on_finish_audit() {
    	var vinno = $('#vinno').val(); var body_no = $('#body_no').val();
        if (vinno != '') {
            var c = confirm('Are you sure vehicle [' + vinno + '] already finish at SQA Check ?');
            if (c) {
                $.post('<?=site_url('t_sqa_dfct/finish_audit')?>',{vinno:vinno}, function(html){
                    if (html == 1) {
                        alert('Vehicle [' + vinno + '] Already Finish Checked at SQA');
                        get_vehicle('vinno');
                    ...
                    <section id="widget_vehicle">
                        <table width="100%" border="0">
                            <tr>
                                ...
                                <td width="1%">:</td>
                                <td width="19%">
                                	<input type="text" name="vinno" id="vinno" onclick="this.select();" maxlength="17" />
                                	<span id="panel_vinno"></span>
                                </td>
...
<script type="text/javascript">
    $(function(){
        /* 
         * edited by: irfan.satriadarma@gmail.com 20120507 
         * since there is no neeed bodyno to retrieve data (it's not UNIQUE!)
         * edited again on 20120508 -> BODY_NO harus dibuka dengan aturan sbb:
         * - jika terdapat lebih dari satu VINNO maka ditampilkan pilihan VINNO sbg combobox order by PLOD Desc
         */        
        ...
        $('#vinno').keypress(function(event){
            ...
            if(keycode == '13'){
                if ($('#vinno').val().length == 17) {
                    ...
                    /*
                     * edited by: irfan.satriadarma@gmail.com 201207050922pm
                     * wtf? trick 2 kali ??		
                    // trick 2 kali, bisa jadi lambat. tp ini the best solusi                    
                    $('#body_no').val('');
                    get_vehicle('body_no');
