...
//            }
    ...
    function change() {
        ...
        **/
        ...
        if ($problem_id != '0') {
            ...
            $problem = $this->dm->select('', '', $w, '*', false);
            if (count($problem)>0) {
                //$body_no = $problem[0]->BODYNO;
            	$vinno = $problem[0]->VINNO;
            }
        }
    ...
    /*
     * to retrieve vehicle info on the table T_SQA_VINF
     * edited by: irfan.satriadarma@gmail.com 20120507
     * need to fix -> find data vehicle harusnya by vinno
     * edited again on 20120508 -> boleh by Body_No tapi dengan pengecekan jika ada vinno yg sama maka keluarkan sbg combobox
     * */
    function get_vinf() {
		...
		if ($vin_id == '') {
	        ...
	        $data_vin = $this->dm->select('', 'PLOD DESC', $where, '*');
			...
			if (count($data_vin) == 0) {
				...
				// edited. cek jika terdapat lebih dari satu VINNO untuk get_vinf by Body No.
				if (count($data_vin) > 1) {
					$arr_vinno = array();					
					
					$out_v = '<select id="vinno_id" style="width: 156px" onclick="on_select_vinno_id();"><option value="0">&mdash; Select Frame No &mdash;</option>';
					foreach ($data_vin as $d) {
						$out_v .= '<option value="'.$d->VINNO.'">'.$d->VINNO.'</option>';
					}
					$out_v .= '</select>';
					...
					// jika inspection datenya kosong, ambil dari Database QIS
					if ($insp_pdate == '' || $insp_pdate == null) {
						$cek = $this->load->database('dbqis', true);
						// edited by irfan@arkamaya.com 20130205 08:30 AM
						$dsql = "select top 1 T_DFCT.CHKPDATE, T_DFCT.CHKPTIME, T_DFCT.CHKPSHIFT from T_DFCT 
								where 
									--T_DFCT.LINE = '1' AND 
									T_DFCT.VINNO = '".$vinno."' 
								order by T_DFCT.CHKPDATE asc";
						$dfct_qis = $cek->query($dsql)->result();
...
//						echo $dsql;
						...
						if (count($dfct_qis)>0) {
							...
							/* edited by: irfan.satriadarma@gmail.com 20120507
							 * key edit bukan by body_no tapi by vinno */
							$keys = "VINNO = '" . $vinno . "'";
					...
					$result 	= array(
									$suffix, $bd_seq, $katashiki, $assy_seq, $insp_pdate,
    ...
    /* to retrieve vehicle under sqa and give the selected record highlighted 
     * edited by: irfan.satriadarma@gmail.com 201205070939pm
     * */ 
    function get_vinf_under_sqa() {
        //$bodyno = $_POST['bodyno'];
        //$str_bodyno = ($bodyno != '' && $bodyno != '0') ? " and (BODY_NO = '" . $bodyno . "')" : '';
        //$str_bodyno = '';
        $vinno 		= (isset($_POST['vinno'])) ? $_POST['vinno'] : '';
        $str_vinno 	= ($vinno != '' && $vinno != '0') ? " and (VINNO = '" . $vinno . "')" : '';
        
        $where = "(REG_IN <> '' or REG_IN is not NULL) ";// . $str_vinno;// $str_bodyno;        
        $this->dm->init('T_SQA_VINF', 'BODY_NO');
    ...
    function reg_in() {
        ...
        $vin = $this->dm->select('', '', "VINNO = '" . $vinno . "'");
        $vin = $vin[0];

        $insp_pdate = (isset($vin->INSP_PDATE)) ? $vin->INSP_PDATE : '';
        $insp_sysdate = (isset($vin->INSP_SYSDATE)) ? $vin->INSP_SYSDATE : '';
        $insp_shiftgrpnm = (isset($vin->INSP_SHIFTGRPNM)) ? $vin->INSP_SHIFTGRPNM : '';
    ...
    /*
     * edited by: irfan.satriadarma@gmail.com fungsi untuk regout menggunakan VINNO */
    function reg_out() {
        $this->dm->init('T_SQA_VINF', 'BODY_NO');

        // update status REG_IN jadi get_date()
        // ambil vinno
        $vinno = $_POST['vinno'];//$body_no = $_POST['bodyno'];        
        $keys = "VINNO = '" . $vinno . "'";//$keys = "BODY_NO = '" . $body_no . "'";
        $data = array(
            'REG_OUT' => get_date()
        );
        $this->dm->update($data, $keys);
        
        // get vin terlebih dahulu untuk dapetin stall no
        $this->dm->init('T_SQA_VINF', 'BODY_NO');
        //$vin = $this->dm->select('', '', "BODY_NO = '" . $body_no . "'");
        $vin = $this->dm->select('', '', "VINNO = '" . $vinno . "'");
        $vin = $vin[0];
    ...
    function get_dfct() {
		...
		$w = "BODYNO = '" . $body_no . "' AND VINNO = '" . $vinno . "' AND (IS_DELETED = '' OR IS_DELETED = '0')";
    ...
    /* edited by: irfansatriadarma@gmail.com 201205071006pm */
    function finish_audit() {
        $vinno = $_POST['vinno']; //$body_no = $_POST['body_no'];
        $this->dm->init('T_SQA_VINF', 'BODY_NO');
        //$vin = $this->dm->select('', '', "BODY_NO = '" . $body_no . "'");
        $vin = $this->dm->select('', '', "VINNO = '" . $vinno . "'");
        $out = 0;
