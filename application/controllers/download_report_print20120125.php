...
class download_report_print extends CI_Controller
{
      ...
      function daily_report()
    {
        ...
        $kurang = strtotime("-35 day");
        $sekarang2 = date("Y-m-d");
        $lastfiveweeks = date("Y-m-d", $kurang);
        ...
        for ($i = 1; $i <= $selisih; $i++)
        {
            ...
            where                      
            c.DESCRIPTION = '" . $model . "'
            and (a.SQA_PDATE >= '" . $lastfiveweeks . "' AND a.SQA_PDATE <= '" .
            $sekarang2 . "')
      ...
      //  $krgbln = strtotime($kurangbulan);
        ...
        for ($i = 0; $i <= 6; $i++)
        {
            $bulan_chart1[] = date("M Y", mktime(0, 0, 0, date("m") - 7 + $i, date("d")-1, date("Y")));
        ...
        $str_pdate = date("M Y", strtotime($T_graph[0]->PDATE));
      ...
      //  $plot->line->stacking = 'normal';
       ...
       // $this->highcharts->set_legend($legend);
        ...
        from T_SQA_DU_SUMMARY_MDL
        where MODEL = '". $model ."' and MONTH(PDATE) = '". $bulan ."'";
        $ambil = $this->dm->sql_self($sql);
       ...
       // Total Zero Defect. Cummulative day which no defect found at the current month.																
        $this->dm->init('T_SQA_DU_SUMMARY_MDL', 'PLANT_CD');
        $w = "PDATE = (select max(PDATE) from T_SQA_DU_SUMMARY_MDL where MODEL = '" . $model . "') and MODEL = '" . $model . "'";
        $ambil = $this->dm->select('', '', $w);
        ...
        from M_SQA_WORK_CALENDAR
        where WORK_FLAG ='1' and SHIFTNO ='2' and month(M_SQA_WORK_CALENDAR.WORK_PRDT) = '". $bulan ."'";
        $ambil = $this->dm->sql_self($sql);
        ...
        from V_T_SQA_DFCT
        where INSP_ITEM_FLG ='1' and month(SQA_PDATE) = '". $bulan ."'
        ...
        from V_T_SQA_DFCT
        where REPAIR_FLG ='1'  and month(SQA_PDATE) = '". $bulan ."'
        and DESCRIPTION = '". $model ."'
        ";
        $ambil = $this->dm->sql_self($sql);
        ...
        from V_T_SQA_DFCT
        where month(SQA_PDATE) = '". $bulan ."' and DESCRIPTION = '". $model ."'
        and ASSY_SHIFTGRPNM = '2'
        ";
        $ambil = $this->dm->sql_self($sql);
