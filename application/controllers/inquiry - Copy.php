...
class inquiry extends CI_Controller
{
    ...
    function search()
    {
        ...
        if ($param != '')
        {
            ...
            if ($sqa_shiftgrpnm != '0')
            {
                $condition .= ($condition != '') ? " AND (SQA_SHIFTGRPNM = '" . $sqa_shiftgrpnm .
                    "' OR AUDIT_SHIFTGRPNM = '" . $sqa_shiftgrpnm ."' )" : " 
                    (SQA_SHIFTGRPNM = '" . $sqa_shiftgrpnm . "' OR AUDIT_SHIFTGRPNM = '" . $sqa_shiftgrpnm ."' )";
            }
            if ($plant_nm != '0')
            {
                $condition .= ($condition != '') ? " AND (PLANT_NM = '" . $plant_nm . "')" :
                    " (PLANT_NM = '" . $plant_nm . "')";
            }
            if ($SHOP_NM != '0')
            {
                $condition .= ($condition != '') ? " AND (SHOP_NM = '" . $SHOP_NM . "')" :
                    " (SHOP_NM = '" . $SHOP_NM . "')";
            }
        }        
        // echo $condition;
        ...
        $sql = "select distinct BODYNO,VINNO, PLANT_NM, IDNO, BODYNO, REFNO,AUDIT_PDATE, DFCT = 'UNDER SQA CHECK',SHOP_NM, DESCRIPTION,
        SQA_SHIFTGRPNM='-',EXTCLR,SQA_SHIFTGRPNM,AUDITOR_NM_1,AUDITOR_NM_2
        from V_T_SQA_DFCT_2
        where APPROVE_PDATE is null and " . $condition . " order by AUDIT_PDATE desc";
        ...
        $non_sqa = $user_grpauth != '04' && $user_grpauth != '05' && $user_grpauth != '06' && $user_grpauth !=
            '07' && $user_grpauth != '08' && $user_grpauth != '09';

        if ($non_sqa)
        {        
            $condition = $condition . " AND ( APPROVE_PDATE IS NOT NULL OR DFCT IS NULL) AND (APPROVE_SYSDATE IS NOT NULL)";
            $list_v_sqa_dfct = $this->v_sqa_dfct_model->select('', $order, $condition, '*', 0);
    ...
    function advance_search()
    {
        ...
        if ($param != '')
        {
            ...
            {
                $condition .= " AND ( (ASSY_PDATE) >= '" . $ASSY_FROM_PDATE .
                    "' AND (ASSY_PDATE) <= '" . $ASSY_TO_PDATE . "')";
            }
            if ($ASSY_SHIFTGRPNM != '0')
            {
                $condition .= ($condition != '') ? " AND (ASSY_SHIFTGRPNM like '%" . $ASSY_SHIFTGRPNM .
                    "%')" : " (ASSY_SHIFTGRPNM like '%" . $ASSY_SHIFTGRPNM . "%')";
            }
            if ($INSP_FROM_PDATE != '' && $INSP_TO_PDATE != '')
            {
                $condition .= " AND ( (INSP_PDATE) >= '" . $INSP_FROM_PDATE .
                    "' AND (INSP_PDATE) <= '" . $INSP_TO_PDATE . "')";
            }
            if ($INSP_SHIFTGRPNM != '0')
            {
                $condition .= ($condition != '') ? " AND (INSP_SHIFTGRPNM like '%" . $INSP_SHIFTGRPNM .
                    "%')" : " (INSP_SHIFTGRPNM like '%" . $INSP_SHIFTGRPNM . "%')";
            }
            if ($DESCRIPTION != '')
            {
                $condition .= ($condition != '') ? " AND (DESCRIPTION like '%" . $DESCRIPTION .
                    "%')" : " (DESCRIPTION like '%" . $DESCRIPTION . "%')";
            }
            if ($KATASHIKI != '')
            {
                $condition .= ($condition != '') ? " AND (KATASHIKI like '%" . $KATASHIKI .
                    "%')" : " (KATASHIKI like '%" . $KATASHIKI . "%')";
            }
            if ($EXTCLR != '')
            {
                $condition .= ($condition != '') ? " AND (EXTCLR like '%" . $EXTCLR . "%')" :
                    " (EXTCLR like '%" . $EXTCLR . "%')";
            }
            if ($VINNO != '')
            {
                $condition .= ($condition != '') ? " AND (VINNO like '%" . $VINNO . "%')" :
                    " (VINNO like '%" . $VINNO . "%')";
            }
            if ($BODYNO != '')
            {
                $condition .= ($condition != '') ? " AND (BODYNO like '%" . $BODYNO . "%')" :
                    " (BODYNO like '%" . $BODYNO . "%')";
            }
            if ($stat_prob == '1')
            {
                $condition .= ($condition != '') ?
                    " AND (SQPR_NUM IS NULL AND PRB_SHEET_NUM IS NOT NULL)" :
                    " (SQPR_NUM IS NULL AND PRB_SHEET_NUM IS NOT NULL)";
            }
            if ($stat_prob == '2')
            {
                $condition .= ($condition != '') ?
                    " AND (SQPR_NUM IS NOT NULL AND PRB_SHEET_NUM IS NOT NULL)" :
                    " (SQPR_NUM IS NOT NULL AND PRB_SHEET_NUM IS NOT NULL)";
            }
            if ($stat_prob == '3')
            {
                $condition .= ($condition != '') ?
            ...
            {
                $condition .= ($condition != '') ?
                    " AND (REG_IN IS NOT NULL AND REG_OUT IS NULL)" :
                    " (REG_IN IS NOT NULL AND REG_OUT IS NULL)";
            }
            if ($stat_prob == '5')
            {
                $condition .= ($condition != '') ? " AND (SHOP_NM = 'Chosagoumi' AND DFCT IS NOT NULL)" :
                    " (SHOP_NM = 'Chosagoumi' AND DFCT IS NOT NULL)";
            }
            if ($DFCT != '')
            {
                $condition .= ($condition != '') ? " AND (DFCT like '%" . $DFCT . "%')" :
                    " (DFCT like '%" . $DFCT . "%')";
            }
            if ($RANK_NM != '0')
            {
            ...
            if ($CTG_GRP_NM != '0')
            {
                $condition .= ($condition != '') ? " AND (CTG_GRP_NM like '%" . $CTG_GRP_NM .
                    "%')" : " (CTG_GRP_NM like '%" . $CTG_GRP_NM . "%')";
            }
          
            if ($CTG_NM != '0')
            {
                $condition .= ($condition != '') ? " AND (CTG_NM like '%" . $CTG_NM . "%')" :
                    " (CTG_NM like '%" . $CTG_NM . "%')";
            }
            if ($INSP_ITEM_FLG != '')
            {
                $condition .= ($condition != '') ? " AND (INSP_ITEM_FLG like '%" . $INSP_ITEM_FLG .
                    "%')" : " (INSP_ITEM_FLG like '%" . $INSP_ITEM_FLG . "%')";
            }
            if ($QLTY_GT_ITEM != '')
            {
                $condition .= ($condition != '') ? " AND (QLTY_GT_ITEM like '%" . $QLTY_GT_ITEM .
                    "%')" : " (QLTY_GT_ITEM like '%" . $QLTY_GT_ITEM . "%')";
            }
            if ($REPAIR_FLG != '')
            {
                $condition .= ($condition != '') ? " AND (REPAIR_FLG like '%" . $REPAIR_FLG .
                    "%')" : " (REPAIR_FLG like '%" . $REPAIR_FLG . "%')";
            }

            if ($Problem_Sheet_a != '')
            {
                $condition .= ($condition != '') ? " AND (PRB_SHEET_NUM like '%" . $Problem_Sheet_a .
                    "%' OR SQPR_NUM like '%" . $Problem_Sheet_a . "%' )" : " (PRB_SHEET_NUM like '%" .
                    $Problem_Sheet_a . "%' OR SQPR_NUM like '%" . $Problem_Sheet_a . "%')";
            }
            if ($Problem_Sheet_b != '')
            {
                $condition .= ($condition != '') ? " AND (PRB_SHEET_NUM like '%" . $Problem_Sheet_b .
                    "%' OR SQPR_NUM like '%" . $Problem_Sheet_b . "%' )" : " (PRB_SHEET_NUM like '%" .
                    $Problem_Sheet_b . "%' OR SQPR_NUM like '%" . $Problem_Sheet_b . "%')";
            }
            if ($Problem_Sheet_c != '')
            {
                $condition .= ($condition != '') ? " AND (PRB_SHEET_NUM like '%" . $Problem_Sheet_c .
                    "%' OR SQPR_NUM like '%" . $Problem_Sheet_c . "%' )" : " (PRB_SHEET_NUM like '%" .
                    $Problem_Sheet_c . "%' OR SQPR_NUM like '%" . $Problem_Sheet_c . "%')";
            }
            if ($Problem_Sheet_d != '')
            {
                $condition .= ($condition != '') ? " AND (PRB_SHEET_NUM like '%" . $Problem_Sheet_d .
                    "%' OR SQPR_NUM like '%" . $Problem_Sheet_d . "%' )" : " (PRB_SHEET_NUM like '%" .
                    $Problem_Sheet_d . "%' OR SQPR_NUM like '%" . $Problem_Sheet_d . "%')";
            }
           
            if ($Status_Problem_Sheet == '0')
            {
                $condition .= ($condition != '') ? " AND (CLOSE_FLG like '%" . $Status_Problem_Sheet .
                    "%')" : " (CLOSE_FLG like '%" . $Status_Problem_Sheet . "%')";
            }
            ...
            if ($Status_Problem_Sheet == '2')
            {
               ...
               (PROBLEM_ID IN (select PROBLEM_ID FROM T_SQA_DFCT_REPLY WHERE REPLY_TYPE = 'R' AND COUNTERMEASURE_TYPE = 'F' AND APPROVE_PDATE is null ))" :
                    " (PROBLEM_ID IN (select PROBLEM_ID FROM T_SQA_DFCT_REPLY WHERE REPLY_TYPE = 'R' AND COUNTERMEASURE_TYPE = 'F' AND APPROVE_PDATE is null))";
            }
            if ($Status_Problem_Sheet == '3')
            {
                $condition .= ($condition != '') ? " AND
               (PROBLEM_ID IN (select PROBLEM_ID FROM T_SQA_DFCT_REPLY WHERE REPLY_TYPE = 'R' AND COUNTERMEASURE_TYPE = 'F' AND APPROVE_PDATE is not null AND DUE_DATE < (select PDATE FROM M_SQA_PRDT )))" :
                    " (PROBLEM_ID IN (select PROBLEM_ID FROM T_SQA_DFCT_REPLY WHERE REPLY_TYPE = 'R' AND COUNTERMEASURE_TYPE = 'F' AND APPROVE_PDATE is not null AND DUE_DATE < (select PDATE FROM M_SQA_PRDT )))";
            }
            if ($SHOW_FLG != '')
            {
                $condition .= ($condition != '') ? " AND (SHOW_FLG like '%" . $SHOW_FLG . "%')" :
                    " (SHOW_FLG like '%" . $SHOW_FLG . "%')";
            }
    ...
    function cek()
    {
        ...
        if ($PDATE != '')
        {
            ...
            $data = array('CHECK_SYSDATE' => get_date(), 'CHECKED_BY' => get_user_info($this,
                'USER_ID'), 'CHECK_PDATE' => $PDATE);
        } else
        {

            $data = array('CHECK_SYSDATE' => get_date(), 'CHECKED_BY' => get_user_info($this,
                'USER_ID'), 'CHECK_PDATE' => get_date());
        }
      ...
      // cek concurency
        ...
        if ($dfct->CHECK_PDATE == '') {
            ...
            if ($dfct->SHOP_NM == 'Chosagoumi') {
                ...
                $data = array('PRB_SHEET_NUM' => $PRB, 
                                'APPROVE_SYSDATE' => $approve_sysdate,
    ...
    function Unapproved()
    {
        ...
        if($approve_pdate !=''){
            ...
            } else
            {
                ...
                $data = array('APPROVE_SYSDATE' => null, 'APPROVED_BY' => null, 'APPROVE_PDATE' => null,
                    'SHOW_FLG' => 0, 'PRB_SHEET_NUM' => null, 'SQPR_NUM' => null);
                $this->dm->update($data, $keys);
                ...
                foreach ($hapus as $ii):
                    ...
                    foreach ($list_reply_attch as $l) {
                        if (file_exists(PATH_ATTCH . $l->ATTACH_DOC)) unlink(PATH_ATTCH . $l->ATTACH_DOC);
                    }
    ...
    function PSclosed()
    {
        ...
        if ($dfct->APPROVE_PDATE != '' && $dfct->CLOSE_FLG == '0' && $dfct_reply->
            APPROVE_PDATE != '')
        {
            $data = array('CLOSE_FLG' => 1, 'CLOSE_PDATE' => get_date(), 'CLOSE_SYSDATE' =>
                get_date(), 'CLOSED_BY' => get_user_info($this, 'USER_ID'));
            $this->dm->update($data, $keys);
        } else
        {
    ...
    function result_search()
    {
        ...
        $this->highcharts->set_title('Result Search SQA System Graph', ''); // set chart title: title, subtitle(optional)
