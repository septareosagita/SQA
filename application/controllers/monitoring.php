...
class monitoring extends CI_Controller
{
    ...
    function show()
    {
        ...
        $err = '';
        $plant_nm = ($this->uri->segment(3) != '') ? $this->uri->segment(3) :
            get_user_info($this, 'PLANT_NM');
        $plantcd = get_user_info($this, 'PLANT_CD');
        if ($plant_nm == '') {
            $err = 1;
        }
        $sql = "select distinct DESCRIPTION from V_T_SQA_DFCT WHERE SHOW_FLG = '1' AND IS_DELETED = '0' AND PLANT_NM = '" .
            $plant_nm . "' ORDER BY DESCRIPTION ASC";
        $data['katashiki'] = $this->dm->sql_self($sql);

        $this->dm->init('V_T_SQA_DFCT', 'PROBLEM_ID');
        $data['dfct_show'] = $this->dm->select('', 'DESCRIPTION ASC, SQA_PDATE DESC',
            "SHOW_FLG = '1' AND IS_DELETED = '0' AND PLANT_NM = '" . $plant_nm . "'");

        // find running text
        $this->dm->init('V_SQA_RUNNING_TEXT', 'PLANT_CD');
        $tgl_now = get_date();
        $w = "PLANT_NM = '" . $plant_nm . "' and (DATE_FROM <= '" . $tgl_now .
            "' and DATE_TO >= '" . $tgl_now . "')";
        $rt = $this->dm->select('', '', $w);
    ...
    function show_sps()
    {
        ...
        */
        ...
        $du = $this->dm->select('DU_TARGET', '', "PLANT_CD = '" . $plantcd .
            "' and (DATE_FROM <= '" . $tgl . "' AND DATE_TO >=  '" . $tgl . "')", '*');
        ...
        $u = $this->dm->select('', '', "PLANT_CD = '" . $plantcd . "' and MODEL='" . $vk .
            "' AND PDATE='" . $pd . "'", 'DU_DAILY, DU_MONTHLY');
    ...
    function du_chart()
    {
        ...
        for ($i = 0; $i < $jml_work_calender; $i++) {
            ...
            //echo '<pre>'; print_r($l_graphA); echo '</pre>';
            ...
            if ($l_graph[0]->DU_MONTHLY != '') {
                ...
                if ($tgl_work_calender[$i] > $max_pdate) {
                    $l_graphDU[$i] = null;
                } else
                    if ($l_graph[0]->DU_MONTHLY == '') {
        ...
        $this->highcharts->set_title('Trend Daily D/U - ' . $model, 'DU Target = <b><blink>[ ' . $du_target .
            ' ]</blink></b> '); // set chart title: title, subtitle(optional)
        ...
        $list_pie = array();
        // PIE chart
        $sql = "select SHOP_NM, SUM(DU_MDL_DAILY) AS NILAI 
        ...
        $sql = "select  SUM(DU_MDL_DAILY) AS TOTAL
        from T_SQA_DU_SUMMARY_MDL_SHOP
        where PDATE = '" . $date_daily_report . "'
        and MODEL = '" . $model . "'  
            ";
        ...
        if ($rt != '') {
            $out = '<div style="border: 1px solid #B9B9B9; 
                        ...
                        <marquee behavior="scroll" scrollamount="3" direction="left" width="100%">
                            <div style="padding: 15px 0 20px 0;">' . $rt->
                RUNTEXT . '</div>
                        </marquee>
                    </div>
                    <input type="text" id="last_rt_dt" value="' . date('Y-m-d H:i:s',
                strtotime($rt->Updatedt)) . '" />
                    ';
        }
        echo $out;
    }
    ...
    function show_du()
    {
        ...
        $sql = "select distinct DESCRIPTION from V_T_SQA_DFCT WHERE SHOW_FLG = '1' AND IS_DELETED = '0' AND PLANT_NM = '" .
            $plant_nm . "' ORDER BY DESCRIPTION ASC";
        $sql = "select distinct DESCRIPTION from V_T_SQA_DFCT WHERE IS_DELETED = '0' AND PLANT_NM = '" .
            $plant_nm . "' ORDER BY DESCRIPTION ASC";
        $data['katashiki'] = $this->dm->sql_self($sql);
