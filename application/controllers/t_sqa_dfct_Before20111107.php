		...
		if ($vin_id == '') {
			...
			if (count($data_vin) == 0) {
                ...
                if ($insp_pdate == '' || $insp_pdate == null) {
                    ...
                    $dfct_qis = $cek->query("select top 1 T_DFCT.CHKPDATE, T_DFCT.CHKPTIME, T_DFCT.CHKPSHIFT from T_DFCT where T_DFCT.LINE = '1' AND T_DFCT.BODY_NO = '".$body_no."' order by T_DFCT.CHKPDATE asc")->result();
                    if (count($dfct_qis)>0) {
...
//        $problem_id = '1ea0c1c6-9fda-4851-82f2-7888cf442e6d';
    ...
    function report_sqa() {
        ...
        if (count($dfct) > 0) {
            ...
            $doc_att_sqpr = $att_sqpr[0]->SQPR_DOC;
